...
#if defined(NDEBUG)
# error "JERUSALEM cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4072d829ccfcfbfab4fa14b6db7e1ae66eeb32e4ccfe8b9edf292c8a7de8c071");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // JERUSALEM: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "JERUSALEM Signed Message:\n";
...
{
    // JERUSALEM: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 33 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 143999 * COIN;
            }

    // Subsidy is cut in half every 21818 blocks
    nSubsidy >>= (nHeight / 21818);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // JERUSALEM: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // JERUSALEM: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1542683467, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Lord Jesus Christ the Royal Bloodline of Adam and Eve is Heir of Earth";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 33 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d1f55fe667c0cf796cb7b20e017ab504f7758af65a8278b59a3e01e9261caf2572d0b7915f889256d6279c3d5c4692fc0ecd0da6cb44e557939c0de040881d0b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1542683467;
        ...
        assert(block.hashMerkleRoot == uint256("0xc0fc0b207901662bd210eea392dcc89fea51037158b126542595283a48de59d1"));
...
unsigned char pchMessageStart[4] = { 0x95, 0x40, 0x2f, 0x33 }; // JERUSALEM: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JERUSALEMMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JERUSALEMMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JERUSALEMMiner(CWallet *pwallet)
{
    printf("JERUSALEMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jerusalem-miner");
    ...
    try { loop {
        ...
        printf("Running JERUSALEMMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JERUSALEMMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JERUSALEMMiner, pwallet));
}
